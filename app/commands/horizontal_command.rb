...
class HorizontalCommand < Command
  def validate(bitmap, args)
    ...
    raise ValidationError, 'Arg X1 is invalid' unless bitmap.valid_index?(args[1])
    raise ValidationError, 'Arg X2 is invalid' unless bitmap.valid_index?(args[2])
    raise ValidationError, 'Arg Y is invalid' unless bitmap.valid_index?(1, args[3])
  end
